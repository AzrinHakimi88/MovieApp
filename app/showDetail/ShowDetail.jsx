...
const ShowDetail = ({id}) => {
    ...
    const [isLoading, error, data, fetchData] = fetchMovieDetail({type: 'show', id: id });
    ...
    if (!data || !data.show || !data.show.poster_path || !data.show.backdrop_path) {
      return null; // Render nothing if data is not available or poster_path is missing
    }
  
    const movieDetail = data.show;
